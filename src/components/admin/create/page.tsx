...
export default function CreateArticlePage() {
  ...
  const createAuthor  = useMutation(api.authors.createAuthor); // ✅ moved to top level
  ...
  // ✅ Store both the preview URL and storageId
  const handleImageUpload = (url: string, storageId: string) => {
    setFeaturedImage(url); // Store blob URL for preview display
    setFeaturedImageId(storageId); // Store storageId to save to DB
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!selectedCategory) { setError('Selected category not found.'); return; }

      // ✅ Get existing author or create one — no hook call inside function
  ...
  return (
    <div style={{ background: '#070f09', minHeight: '100vh', fontFamily: '"Helvetica Neue", Arial, sans-serif', color: '#e8dfc8' }}>

      {/* Top bar */}
      <div style={{
        position: 'sticky', top: 0, zIndex: 50,
        background: 'rgba(7,15,9,0.95)', backdropFilter: 'blur(12px)',
        borderBottom: '1px solid rgba(201,168,76,0.1)', padding: '0 16px',
      }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 14, height: 54 }}>
          ...
          <button onClick={() => { setStatus('draft'); handleSubmit(); }} style={{
          ...
          <button onClick={handleSubmit} disabled={saving} style={{
            background: isReady ? (saving ? 'rgba(201,168,76,0.5)' : 'linear-gradient(135deg, #b8942a, #d4aa48)') : 'rgba(255,255,255,0.05)',
            ...
            display: 'flex', alignItems: 'center', gap: 7, transition: 'filter 0.15s',
          }}
            onMouseEnter={e => { if (isReady && !saving) e.currentTarget.style.filter = 'brightness(1.1)'; }}
            onMouseLeave={e => { e.currentTarget.style.filter = 'none'; }}
          >
            {saving ? (<><span style={{ width: 12, height: 12, border: '2px solid #070f09', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} />Publishing…</>) :
             saved  ? (<><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>Published!</>) :
                      (<><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Publish</>)}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(20px,3vw,36px) 16px 60px' }}>
        <div className="editor-grid">
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 24, fontWeight: 700, color: '#f0e8d0', letterSpacing: '-0.3px' }}>Write New Article</h1>
              </div>
              <p style={{ fontSize: 12, color: '#2e5a3a', letterSpacing: '0.04em', paddingLeft: 11 }}>Compose, format, and publish your story.</p>
            </div>

            {/* Headline */}
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 7 }}>
                <Label>Headline</Label><CharCount value={title} max={120} />
              </div>
              <textarea value={title} onChange={e => setTitle(e.target.value)}
                placeholder="Write a clear, compelling headline…" rows={2} maxLength={120}
                style={{ ...field, fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 22, fontWeight: 700, lineHeight: 1.35, resize: 'none', letterSpacing: '-0.2px' }}
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 7 }}>
                <Label>Excerpt / Standfirst</Label><CharCount value={excerpt} max={280} />
              </div>
              <textarea value={excerpt} onChange={e => setExcerpt(e.target.value)}
                placeholder="A brief summary shown in article cards and search results (1–2 sentences)…"
                rows={3} maxLength={280} style={{ ...field, resize: 'vertical' }}
                onFocus={onFocus} onBlur={onBlur} />
            </div>
            ...
            <div>
              <div style={{
                ...
                borderBottom: 'none', borderRadius: '4px 4px 0 0', padding: '4px 8px', flexWrap: 'wrap',
              }}>
                <Label>Body</Label>
                <div style={{ flex: 1 }} />
                {[
                ...
                ].map(tb => <ToolBtn key={tb.label} label={tb.label} icon={tb.icon} onClick={tb.action} />)}
              ...
              <textarea ref={contentRef} value={content} onChange={e => setContent(e.target.value)}
                ...
                rows={22} style={{ ...field, borderRadius: '0 0 4px 4px', resize: 'vertical', lineHeight: 1.85, fontFamily: '"Courier New", Courier, monospace', fontSize: 14, minHeight: 400 }}
                onFocus={onFocus} onBlur={onBlur} />
            ...
            <div style={{ marginTop: 24, padding: '18px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.08)', borderRadius: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                {tags.map(t => (
                  <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.22)', borderRadius: 3, padding: '3px 9px', fontSize: 11, color: '#c9a84c', fontWeight: 600 }}>
                    #{t}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input type="text" value={tagInput} onChange={e => setTagInput(e.target.value)}
                  ...
                  onFocus={onFocus} onBlur={onBlur} />
                <button onClick={addTag} style={{ background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.22)', borderRadius: 4, padding: '0 14px', color: '#c9a84c', fontSize: 11, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', whiteSpace: 'nowrap' }}>Add</button>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Stats */}
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid rgba(201,168,76,0.06)' }}>
                <Stat icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>} label="Words" value={wordCount} />
                <div style={{ width: 1, background: 'rgba(201,168,76,0.06)' }} />
                <Stat icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} label="Min Read" value={readTime} />
                <div style={{ width: 1, background: 'rgba(201,168,76,0.06)' }} />
                <Stat icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>} label="Chars" value={content.length} />
              </div>
              <div style={{ padding: '12px 14px' }}>
                ...
                <div style={{ height: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg, #b8942a, #d4aa48)', width: `${([title, excerpt, content, category, featuredImage, images.length > 0 ? 'yes' : ''].filter(Boolean).length / 6) * 100}%`, transition: 'width 0.3s ease' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 10 }}>
                  {[
                    ...
                    { label: 'Article Images', done: images.length > 0 },
                  ].map(item => (
                    <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 11 }}>
                      <span style={{ width: 14, height: 14, borderRadius: '50%', background: item.done ? 'rgba(201,168,76,0.15)' : 'rgba(255,255,255,0.04)', border: item.done ? '1px solid rgba(201,168,76,0.4)' : '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 8, color: item.done ? '#c9a84c' : '#2a4a30', transition: 'all 0.2s' }}>✓</span>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '14px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
                  {categories?.map(cat => (
                    <button key={cat._id} onClick={() => setCategory(cat.name)} style={{
                      ...
                      cursor: 'pointer', letterSpacing: '0.06em', transition: 'all 0.15s', textAlign: 'left',
                    }}
                ...
                <button onClick={handleSubmit} disabled={saving} style={{
                  width: '100%',
                  background: isReady ? (saving ? 'rgba(201,168,76,0.4)' : 'linear-gradient(135deg, #b8942a, #d4aa48)') : 'rgba(255,255,255,0.04)',
                  ...
                  cursor: isReady && !saving ? 'pointer' : 'not-allowed',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'filter 0.15s',
                }}
                  onMouseEnter={e => { if (isReady && !saving) e.currentTarget.style.filter = 'brightness(1.08)'; }}
                  onMouseLeave={e => { e.currentTarget.style.filter = 'none'; }}
                >
                  {saving ? (<><span style={{ width: 13, height: 13, border: '2px solid #070f09', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} />Publishing…</>) :
                   saved  ? (<><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>Published!</>) :
                            (<><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Publish Article</>)}
                </button>
                {!isReady && <p style={{ fontSize: 10, color: '#3a5a3a', marginTop: 8, textAlign: 'center', lineHeight: 1.5 }}>Fill in headline, body &amp; category to publish</p>}
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '14px' }}>
                <ImageUpload
                  ...
                  currentImage={undefined} // ✅ never pass a blob URL as currentImage
                  label="Upload Photo"
                />
                {featuredImageId && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 3, padding: '6px 10px' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '14px' }}>
                <div style={{ background: '#fff', borderRadius: 4, padding: '12px 14px', border: '1px solid #e0e0e0' }}>
                  <div style={{ fontSize: 11, color: '#006621', marginBottom: 3, letterSpacing: '0.02em' }}>hirwaambassadeur.rw › article</div>
                  <div style={{ fontSize: 14, color: '#1a0dab', fontWeight: 500, lineHeight: 1.3, marginBottom: 4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                    {title || 'Article headline will appear here…'}
                  </div>
                  <div style={{ fontSize: 12, color: '#545454', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                    {excerpt || 'Your article excerpt will appear as the meta description in search results.'}
                  </div>
                </div>
                <p style={{ fontSize: 10, color: '#2a4a30', marginTop: 8, lineHeight: 1.5 }}>The headline and excerpt populate your Google search preview.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Toast */}
      {saved && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 999, background: 'linear-gradient(135deg, #0b1e10, #162d1c)', border: '1px solid rgba(201,168,76,0.3)', borderRadius: 6, padding: '12px 24px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', animation: 'fadeUp 0.3s ease both' }}>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .editor-grid { display: grid; grid-template-columns: 1fr; gap: 24px; }
        @media (min-width: 860px) { .editor-grid { grid-template-columns: minmax(0,1fr) 300px; gap: 28px; } }
      `}</style>
    </div>
  );
}