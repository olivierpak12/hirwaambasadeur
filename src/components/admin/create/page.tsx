                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
                  {CATEGORIES.map(cat => (
                    <button key={cat} onClick={() => setCategory(cat)} style={{
                      background: category === cat ? 'rgba(201,168,76,0.12)' : 'rgba(255,255,255,0.02)',
                      border: category === cat ? '1px solid rgba(201,168,76,0.4)' : '1px solid rgba(201,168,76,0.08)',
                      borderRadius: 3, padding: '7px 8px',
                      color: category === cat ? '#c9a84c' : '#3a6a4a',
                      fontSize: 11, fontWeight: category === cat ? 800 : 500,
                      cursor: 'pointer', letterSpacing: '0.06em',
                      transition: 'all 0.15s',
                      textAlign: 'left',
                    }}
                      onMouseEnter={e => { if (category !== cat) { e.currentTarget.style.borderColor = 'rgba(201,168,76,0.2)'; e.currentTarget.style.color = '#5a8a6a'; }}}
                      onMouseLeave={e => { if (category !== cat) { e.currentTarget.style.borderColor = 'rgba(201,168,76,0.08)'; e.currentTarget.style.color = '#3a6a4a'; }}}
                    >{cat}</button>
                  ))}
                </div>
